...
#' @param par.method  Type of method to estimate CDMs' parameters; one out of \code{"EM"}, \code{"BM"}. Default = \code{"EM"}. 
...
#'   \describe{
#'     \item{"ESA"}{for exhaustive search algorithm. Cannot be used with the \code{"Wald"} method.}
...
#'  \item{time.cost}{The time cost to finish the function.}
...
validation <- function(Y, Q, 
  ...
  res <- list(Q.orig = Qval.obj$Q.original, Q.sug = Qval.obj$Q.sug,
              time.cost = time.cost, call=validationCall, 
              arguments = list(
                Y=Y, Q=Q, 
